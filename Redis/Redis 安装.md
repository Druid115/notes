...
# 代表开启守护进程模式。此时是单进程多线程的模式，Redis 将在后台运行，并将 pid 写入 redis.conf--pidfile 文件中。此时 Redis，除非手动 kill。默认为 no，当前界面将进入 Redis 的命令行界面，exit 强制退出或者关闭连接工具（xshell 等）都会导致 Redis 进程退出
daemonize yes
...
# 缓冲区写入磁盘的频率 always 每次写入都会立即同步；everysec 每秒同步；no 让操作系统来决定合适同步磁盘
